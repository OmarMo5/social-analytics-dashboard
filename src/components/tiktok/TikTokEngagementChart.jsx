...
const TIKTOK_GREEN = '#34d399';
const TIKTOK_AMBER = '#fbbf24';

const CustomTooltip = ({ active, payload, label }) => {
  if (!active || !payload?.length) return null;
  return (
    <div className="card" style={{ padding: '12px 16px', border: '1px solid rgba(37,244,238,.3)' }}>
...
export default function TikTokEngagementChart({ data }) {
    ...
    <div className="card" style={{ padding: '24px 24px 18px' }}>
      <div style={{ marginBottom: 20 }}>
        ...
        <p style={{ fontSize: 12, color: 'var(--text-3)', marginTop: 4 }}>
          المشاهدات والإعجابات والمشاركات شهرياً
        </p>
      </div>
      <ResponsiveContainer width="100%" height={240}>
        ...
        >
          ...
          <YAxis 
            ...
            axisLine={false} 
          />
          <Tooltip content={<CustomTooltip />} cursor={{ fill: 'rgba(37,244,238,.05)' }} />
          <Bar 
            dataKey="views" 
            name="المشاهدات" 
            fill={TIKTOK_CYAN} 
      ...
      <div style={{ display: 'flex', gap: 18, justifyContent: 'center', marginTop: 14, flexWrap: 'wrap' }}>
        {[
          ['المشاهدات', TIKTOK_CYAN],
          ['الإعجابات', TIKTOK_PINK],
          ['المشاركات', TIKTOK_GREEN]
        ].map(([n, c]) => (
